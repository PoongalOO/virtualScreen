...
| 9b | Rotation (tablette physiquement retournée) | Rendu : **OK**, pixel-exact contre `xwd` une fois l'orientation prise en compte (l'app suit le capteur, `sensorLandscape`). Toucher : **défaut trouvé puis corrigé (SS-088)** — une position touchée à l'écran envoyait au serveur la coordonnée **symétrique**, pas celle affichée, faute de compensation par la plateforme dans cette orientation. Corrigé par `TouchRotationQuirk` et **revérifié sur l'appareil dans les deux orientations** : normale inchangée, retournée corrigée dans les deux sens (un toucher atteint désormais la fenêtre visée, et n'atteint plus l'ancien point symétrique). **OK** |
| 10 | Session 30 min | SS-061 : `scripts/reference_session.py`, résultats dans PERFORMANCE.md. **OK** |
| 11 | Session 2 h | Idem. **OK** |

**Un défaut trouvé et corrigé** (rotation/toucher, n° 9b, SS-088), rien d'autre. Deux essais initiaux invalides, corrigés en cours de campagne : un décalage vertical de 48 px dans mon script de comparaison (l'application n'affiche pas de barre système par-dessus l'écran distant, contrairement à mon hypothèse) et une confusion d'index de champ (mot de passe vs port) dans le scénario de reconnexion manuelle — deux erreurs de méthode, aucune du côté de l'application.
